...
export default function ProfilePage() {
  ...
  const [nickname, setNickname] = useState((userAny?.publicMetadata?.nickname as string) || (userAny?.firstName || userAny?.lastName ? `${userAny.firstName} ${userAny.lastName}` : '') || "")
  const [username, setUsername] = useState(userAny?.username || "")
  const [avatarUrl, setAvatarUrl] = useState(userAny?.imageUrl || "")
      ...
      if (user) {
        try {
          ...
          if (response.ok) {
            ...
            // 프로필 완성도 계산 (API에서 반환된 데이터 사용)
            // calculateProfileCompleteness 함수는 여전히 필요하지만, 이제 클라이언트 측에서 Clerk user 객체를 직접 받음
            const completeness = calculateProfileCompleteness(data); // Use data from API
            setProfileCompleteness(completeness);
          } else {
            console.error('Failed to fetch profile data:', response.statusText);
            setMessage({ type: 'error', text: '프로필 정보를 가져오는 데 실패했습니다.' });
          }
        } catch (error) {
          console.error('Error fetching profile data:', error);
          setMessage({ type: 'error', text: '프로필 정보를 가져오는 중 오류가 발생했습니다.' });
        }
      }
    };
    fetchProfile();
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="relative">
                      <Avatar className="h-20 w-20">
                        <AvatarImage 
                          src={avatarUrl || userAny?.imageUrl} 
                          alt={userAny?.username || userAny?.emailAddresses?.[0]?.emailAddress || 'User'}
                        />
                        <AvatarFallback className="bg-primary text-primary-foreground text-2xl">
                          {userAny?.firstName?.charAt(0)?.toUpperCase() || userAny?.emailAddresses?.[0]?.emailAddress?.charAt(0)?.toUpperCase() || '?'}
                        </AvatarFallback>
                      </Avatar>
                      <ImageUpload 
                    ...
                    <div>
                      <CardTitle className="text-2xl">{userAny?.username || userAny?.emailAddresses?.[0]?.emailAddress || 'User'}</CardTitle>
                      <CardDescription className="text-base">{userAny?.emailAddresses?.[0]?.emailAddress}</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                        ...
                        <span>{formatDate(userAny?.created_at || '')}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                        <Calendar className="h-4 w-4" />
                        <span>{formatDate((userAny as any)?.last_sign_in_at || userAny?.created_at || '')}</span>
                      </div>
                    </div>
                  </div>
